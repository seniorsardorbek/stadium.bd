...
export class StadionsService {
  ...
  async create (data: CreateStadionDto, image: Array<Express.Multer.File>) {
    try {
      if (!image?.length)
        throw new BadRequestException({
          message: 'Stadion uchun rasm qatiy!',
          succes: false
        })
      const images = image.map(el => {
        return el.filename
      })
      const newdata = await this.stadionModel
        .create({
          ...data,
          images,
        ...
        .catch(err => {
          ...
          throw new BadRequestException({
            message: "Ma'lumotlar bilan xatolik bor ",
            succes: false,
            err: err.message
          })
        })
      return { message: 'Stadion yaratildi, davom eting...', data: newdata }
    } catch (error) {
      throw new BadRequestException({
  ...
  async findAll ({
    ...
    try {
      ...
      throw new BadRequestException({
        message: "Birozdan so'ng urinib koring...",
        success: false ,
        error 
      })
    }
